            ...
            for K in 1 5; do
                for dataset in BANKING77 HWU64 OOS Liu; do
                    ...
                    backtranslation_params="
                        --n-unlabeled 5
                        --augmentation-data-path data/${dataset}/pointer-paraphrase.jsonl"
                    ...
                    # .--------------------------------.
                    # | ProtAugment + POINTER-paraphrase |
                    # '--------------------------------'
                    OUT_PATH="runs_consistency/DBS-10samp/${dataset}/${cv}/${C}C_${K}K/seed${seed}/pointer-paraphrase"
                    if [[ -d "${OUT_PATH}" ]]; then
                    ...
                    # .-----------------------------------------------.
                    # | ProtAugment + POINTER-paraphrase - full dataset |
                    # '-----------------------------------------------'
                    OUT_PATH="runs_consistency/full_datasets/${dataset}/${cv}/${C}C_${K}K/seed${seed}/pointer-paraphrase"
                    if [[ -d "${OUT_PATH}" ]]; then
                        echo "${OUT_PATH} already exists. Skipping."
                    else
                        mkdir -p ${OUT_PATH}
