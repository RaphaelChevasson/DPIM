...
def convert_example_to_features(example, tokenizer, max_seq_length, id=0, no_ins_at_first=False, no_ins_at_end=False, tokenizing=False):
    tokens = ["[CLS]"] + example
    ...
    if tokenizing:
        ...
        for l in input_ids_lens:
            ...
            cur += l
        if no_ins_at_end == 'when_punctuation_at_end':
            if tokens[-1].endswith(('.', '?', '!')):  # also handle case where last token is '?!' or '???'
                no_ins.append(cur-1)
        elif no_ins_at_end:
            no_ins.append(cur-1)
    else:
...
class PregeneratedDataset(Dataset):
    def __init__(self, training_path, epoch, tokenizer, num_data_epochs, max_seq_len=256, sep=" ",
                 no_ins_at_first=False, no_ins_at_end=False, reduce_memory=False):
        ...
        logging.info(f"Loading training or inference examples for epoch {epoch}")
        with data_file.open() as f:
            for i, line in enumerate(tqdm(f, total=num_samples, desc="Loading examples")):
                if i >= num_samples:
                    break
                line = line.strip()
                example = [s.lstrip().strip() for s in line.split(sep)]
                features = convert_example_to_features(example, tokenizer, seq_len, no_ins_at_first=no_ins_at_first,
                                                       no_ins_at_end=no_ins_at_end, id=i, tokenizing=True)
    ...
    if verbose > 0:
        print("\nInput %s" % (" ".join(
            [str(tokenizer.ids_to_tokens.get(x, "noa").encode('ascii', 'ignore').decode('ascii')) for x in
             input_ids[0].detach().cpu().numpy() if x != 0])))
    ...
    for ip in range(MAX_TURN):
        with torch.no_grad():
            ...
            if verbose == 0:
                ...
                print("Round %d: %s" % (ip, " ".join([str(
                    tokenizer.ids_to_tokens.get(x, "noa").encode('ascii', 'ignore').decode('ascii')) + (
                                                          ("(" + "{:.2f}".format(float(logit_new[0, i])) + ")") if
                                                          logit_new[0, i] > 0 else "") for i, x in
                                                      enumerate(input_ids[0].detach().cpu().numpy()) if x != 0])))
            elif verbose == 3:
                print("Round %d: %s" % (ip, " ".join([str(
                    tokenizer.ids_to_tokens.get(x, "noa").encode('ascii', 'ignore').decode('ascii')) + ((
                                                                                                                    "(" + "{:.2f}".format(
                                                                                                        ...
                                                                                                        logit_new[
                                                                                                            0, i] > 0 else "")
                                                      for i, x in enumerate(input_ids[0].detach().cpu().numpy()) if
                                                      x != 0])))
            else:
                print("Round %d: %s" % (ip, " ".join(
                    [str(tokenizer.ids_to_tokens.get(x, "noa").encode('ascii', 'ignore').decode('ascii')) for x in
                     input_ids[0].detach().cpu().numpy() if x != 0])))
    return input_ids
...
def main(args):
    ...
    parser.add_argument("--no_ins_at_end",
                        type=str,
                        default='false',
                        help="Do not insert at the end of the text. Can be true, false or when_punctuation_at_end.")

    args = parser.parse_args(args)
    try:
        args.no_ins_at_end = {
            'true': True, 'false': False, 'when_punctuation_at_end': 'when_punctuation_at_end'
        }[args.no_ins_at_end.lower()]
    except KeyError:
        raise(KeyError("'--no_ins_at_end' must be 'true', 'false', or 'when_punctuation_at_end'"))
    ...
    epoch_dataset = PregeneratedDataset(epoch=0, training_path=args.keyfile, tokenizer=tokenizer,
                                        max_seq_len=args.max_seq_length, sep=args.sep,
                                        no_ins_at_first=args.no_ins_at_first, no_ins_at_end=args.no_ins_at_end,
                                        num_data_epochs=1)
    ...
    with tqdm(total=len(generate_dataloader), desc=f"Epoch {0}") as pbar:
        for step, batch in enumerate(generate_dataloader):
            ...
            if args.type == "greedy":
                predict_ids = greedy_search(model, input_ids, segment_ids, input_mask, no_ins=no_ins, args=args,
                                            tokenizer=tokenizer, prevent=prevent, reduce=reduce)
            elif args.type == 'sampling':
                predict_ids = sample_generate(model, input_ids, segment_ids, input_mask, no_ins=no_ins, temperature=0.8,
                                              args=args, tokenizer=tokenizer, prevent=prevent, reduce=reduce)
            else:
                raise NotImplementedError

            output = [tokenizer.decode(ids, skip_special_tokens=True) for ids in predict_ids.cpu().detach().numpy()]
